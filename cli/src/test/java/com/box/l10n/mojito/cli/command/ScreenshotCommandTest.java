...
//        getL10nJCommander().run("screenshot", "-r", repository.getName());
        getL10nJCommander().run("screenshot", "-r", "ExScreengrab");
    }

    @Test
    public void execute() throws Exception {
//        Repository repository = createTestRepoUsingRepoService("Screen1");
        String repoName = "aScreenshot" + new Date().getTime();
        ...
        repositoryService.addRepositoryLocale(repository, "en-GB");
...
//        Repository repository = createTestRepoUsingRepoService("Screen1");
        String repoName = "android_" + new Date().getTime();
        Repository repository = repositoryService.createRepository(repoName, repoName + " description");

        repositoryService.addRepositoryLocale(repository, "fr-FR");
        repositoryService.addRepositoryLocale(repository, "ja-JP");

        getL10nJCommander().run("push", "-r", repository.getName(),"-lm" , "fr:fr-FR,ja:ja-JP", "-s",  getInputResourcesTestDir().toString());
        getL10nJCommander().run("import", "-r", repository.getName(),"-lm" , "fr:fr-FR,ja:ja-JP", "-s", getInputResourcesTestDir().toString());
        getL10nJCommander().run("screenshot", "-r", repository.getName(),"-lm" , "fr:fr-FR,ja:ja-JP", "-s", getInputResourcesTestDir().toString());
        getL10nJCommander().run("screenshot", "-r", repository.getName(),"-lm" , "fr:fr-FR,ja:ja-JP", "-s", getInputResourcesTestDir().toString());
    }
